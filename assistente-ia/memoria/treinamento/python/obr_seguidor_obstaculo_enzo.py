...
OBSTACULO_DIST = 12          # Distância de detecção do obstáculo (cm)
OBSTACULO_VEL = 30           # Velocidade de avanço durante o desvio
OBSTACULO_TEMPO_LADO = 1.5    # Tempo para se afastar da linha para o lado (segundos) - Aumentado para andar mais
OBSTACULO_TEMPO_FRENTE = 2.2  # Tempo para avançar reto passando pelo obstáculo (segundos) - Aumentado para andar mais
OBSTACULO_TEMPO_ALINHAMENTO = 0.8  # Tempo (s) avançando após achar a linha para alinhar o eixo das rodas
...
def obstaculo():
    # Evita leituras falsas confirmando a distancia com multiplas medicoes
    leituras_validas = 0
    # Otimização: Só faz as verificações repetidas se a primeira acusar obstáculo!
    if ultra.distance_centimeters < OBSTACULO_DIST:
        leituras_validas += 1
        for _ in range(2):
            sleep(0.01)
            if ultra.distance_centimeters < OBSTACULO_DIST:
    ...
    if leituras_validas == 3:
        print("Obstaculo detectado! Iniciando desvio...")
        tank.off()
        sleep(0.5)

        # 1. Desvio lateral inicial
        # Gira 90° para a direita
        girar_angulo(90)
        # Avança para passar ao lado do obstáculo
        tank.on_for_seconds(OBSTACULO_VEL, OBSTACULO_VEL, OBSTACULO_TEMPO_LADO)
        tank.off()
        sleep(0.2) # Estabiliza antes do próximo giro
        
        # 2. Contorna o obstáculo lateralmente
        # Gira 90° para a esquerda
        girar_angulo(-90)
        # Avança passando pelo obstáculo
        tank.on_for_seconds(OBSTACULO_VEL, OBSTACULO_VEL, OBSTACULO_TEMPO_FRENTE)
        tank.off()
        sleep(0.2)
        
        # Gira 90° para a esquerda para voltar em direção à linha
        girar_angulo(-90)
        
        # 3. Busca Inteligente da Linha (Malha Fechada)
        tank.on(15, 15)  # Velocidade mais lenta para parar com precisão
        tempo_busca = time.time()
        while True:
            ref_e, ref_d = ler_sensores()
            # Se algum sensor encontrar a linha preta, interrompe a busca
            if ref_e < LIMIAR_PRETO or ref_d < LIMIAR_PRETO:
                break
            # Segurança: se demorar mais de 6 segundos buscando, aborta
            if time.time() - tempo_busca > 6.0:
                print("Aviso: Timeout na busca de linha pos-obstaculo!")
                break

            sleep(0.01)
        
        # Avança um pouco mais para alinhar o eixo das rodas (centro de rotação) sobre a linha
        tank.on_for_seconds(15, 15, OBSTACULO_TEMPO_ALINHAMENTO)
        tank.off()
        sleep(0.2)
        
        # 4. Alinhamento final
        # Gira 90° para a direita para alinhar o corpo na direção da pista
        print("Linha reencontrada! Alinhando 90 graus para a direita...")
        girar_angulo(90)
...
while True:
    ...
    # 2. Faz uma única leitura RGB por ciclo para ambos os sensores
    rgb_e = corE.rgb
    rgb_d = corD.rgb
    
    # 3. Calcula as refletâncias equivalentes
    ref_e = obter_refletancia_rgb(rgb_e)
    ref_d = obter_refletancia_rgb(rgb_d)

    # 4. Checa se chegou na entrada de prata do Resgate
    if ref_e > LIMIAR_PRATA and ref_d > LIMIAR_PRATA:
        resgate()
    else:
        # 5. Segue a linha normalmente usando os marcadores verdes e refletâncias calculadas
