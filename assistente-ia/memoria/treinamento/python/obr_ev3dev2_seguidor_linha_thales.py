...
# variaveis de estado do PID (fora do loop para manter valor entre iteracoes)
erro_anterior = 0
integral = 0
...
while True:
    dist_mm = sensor_ultrasonico.distance_centimeters * 10 # leitura unica do sensor por ciclo
    if dist_mm < 120:  #distancia em mm
        caixa()
    elif sensor_direita.color == ColorSensor.COLOR_WHITE and sensor_esquerda.color == ColorSensor.COLOR_WHITE: # CORRIGIDO: verificacao correta de prata (branco)
        mover_bloco_sgiro(80, 120) #*45cm
        virar_bloco_sgiro(90, "esquerda", 80)
        mover_bloco_sgiro(80, 120) #*45cm

        inicio_resgate = time() # CORRIGIDO: usar time() para controle do timer
        dist_vitima = 30 # Valor inicial padrao para evitar NameError
        while (time() - inicio_resgate) <= temp_resgate:
            if sensor_ultrasonico.distance_centimeters * 10 >= 30 and vitima == 0:
                robot.on(-40, 40)
                dist_vitima = sensor_ultrasonico.distance_centimeters * 10
            else:
                vitima = 1 # CORRIGIDO: era vitima == 1 (comparacao), agora e atribuicao
                robot.off()
                ...
                robot.on(int(40 - (dist_vitima - 5)), int(40 + (dist_vitima - 5)))
                sleep(0.05)
                vitima = 0 # CORRIGIDO: era vitima == 0 (comparacao), agora e atribuicao
        ...
        if sensor_direita.color == ColorSensor.COLOR_RED: #and sensor_esquerda.color() != Color.GREEN:
            ...
            # CORRIGIDO: erro_anterior, integral e constantes PID movidos para fora do loop
            ref_erro = ref_sensor_direita - ref_sensor_esquerda
            integral += ref_erro #integral
            integral = max(-100, min(100, integral)) # anti-windup: limita acumulo do integral
            ...
            velocidade = max(velocidade, 30) # CORRIGIDO: era min+if que travava em 50; agora limita minimo real in curvas
